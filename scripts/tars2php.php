...
class InterfaceParser
{
    ...
    public function getInterfaceBasic()
    {
        ...
        $this->tabSymbol . 'public $getrequest = false;' . $this->returnSymbol .
        $this->tabSymbol . 'public $getresponse = true;' . $this->returnSymbol .
        $this->tabSymbol . 'public $notinvoke = false;' . $this->returnSymbol .
        $this->tabSymbol . 'public $responseBuffer;' . $this->returnSymbol .
        $this->tabSymbol . "public \$_servantName = \"$this->servantName\";" . $this->doubleReturn .
        $this->tabSymbol . 'public function __construct(CommunicatorConfig $config, $notinvoke = false) {' . $this->returnSymbol .

        $this->doubleTab . 'try {' . $this->returnSymbol .
        $this->tripleTab . '$config->setServantName($this->_servantName);' . $this->doubleReturn .
        $this->tripleTab . 'if (!$notinvoke) {' . $this->returnSymbol .
        $this->quardupleTab . '$this->_communicator = new Communicator($config);' . $this->returnSymbol .
    ...
    public function generateFuncBody($inParams, $outParams, $returnInfo)
    {
        ...
        $bodySuffix = ' catch (\\Exception $e) {' . $this->returnSymbol .
        $this->tripleTab . 'throw $e;' . $this->returnSymbol .
        $this->doubleTab . '}' . $this->returnSymbol;

        //修改的代码，增加是否是获取response信息的判断
        $bodyMiddle = $this->tripleTab . 'if (!$this->getresponse || !$this->notinvoke) {' . $this->returnSymbol .
        $this->quardupleTab . '$requestPacket = new RequestPacket();' . $this->returnSymbol .
        $this->quardupleTab . '$requestPacket->_iVersion = $this->_iVersion;' . $this->returnSymbol .
        $this->quardupleTab . '$requestPacket->_funcName = __FUNCTION__;' . $this->returnSymbol .
        $this->quardupleTab . '$requestPacket->_servantName = $this->_servantName;' . $this->returnSymbol .
        $this->quardupleTab . '$encodeBufs = [];' . $this->doubleReturn;
        ...
        foreach ($inParams as $param) {
            ...
            if (Utils::isVector($type)) {
                $vecFill = $this->quardupleTab . '$' . $valueName . '_vec = ' . $this->getExtType($param['wholeType'], $valueName) . ';' . $this->returnSymbol .
                $this->quardupleTab . 'foreach($' . $valueName . ' as ' . '$single' . $valueName . ') {' . $this->returnSymbol .
                $this->tabSymbol . $this->quardupleTab . '$' . $valueName . '_vec->pushBack($single' . $valueName . ');' . $this->returnSymbol .
                $this->quardupleTab . '}' . $this->returnSymbol;
                $bodyMiddle .= $vecFill;
                $bodyMiddle .= $this->quardupleTab . $commonPrefix . $packMethod . '("' . $valueName . "\",{$index},\$" . $valueName . '_vec,$this->_iVersion);' . $this->returnSymbol;
            }

            // 判断如果是map需要特别的处理
            elseif (Utils::isMap($type)) {
                $mapFill = $this->quardupleTab . '$' . $valueName . '_map = ' . $this->getExtType($param['wholeType'], $valueName) . ';' . $this->returnSymbol .
                $this->quardupleTab . 'foreach($' . $valueName . ' as ' . '$key => $value) {' . $this->returnSymbol .
                $this->tabSymbol . $this->quardupleTab . '$' . $valueName . '_map->pushBack([$key => $value]);' . $this->returnSymbol .
                $this->quardupleTab . '}' . $this->returnSymbol;
                $bodyMiddle .= $mapFill;
                $bodyMiddle .= $this->quardupleTab . $commonPrefix . $packMethod . '("' . $valueName . "\",{$index},\$" . $valueName . '_map,$this->_iVersion);' . $this->returnSymbol;
            }
            // 针对struct,需要额外的use过程
            elseif (Utils::isStruct($type, $this->preStructs)) {
                ...
                $bodyMiddle .= $this->quardupleTab . $commonPrefix . $packMethod . '("' . $valueName . "\",{$index},\$" . $valueName . ',$this->_iVersion);' . $this->returnSymbol;
            } else {
                $bodyMiddle .= $this->quardupleTab . $commonPrefix . $packMethod . '("' . $valueName . "\",{$index},\$" . $valueName . ',$this->_iVersion);' . $this->returnSymbol;
            }

            $bodyMiddle .= $this->quardupleTab . "\$encodeBufs['{$valueName}'] = \$__buffer;" . $this->returnSymbol;
        }

        $bodyMiddle .= $this->quardupleTab . '$requestPacket->_encodeBufs = $encodeBufs;' .
        $this->doubleReturn;

        //修改的代码，增加是否是获取request信息的判断
        $bodyMiddle .= $this->quardupleTab . 'if ($this->getrequest) {' . $this->returnSymbol;
        $bodyMiddle .= $this->doubleTab . $this->tripleTab . 'return $requestPacket;' . $this->returnSymbol;
        $bodyMiddle .= $this->quardupleTab . '}' . $this->doubleReturn;

        //修改的代码，增加是否需要执行invoke的判断
        $bodyMiddle .= $this->quardupleTab . 'if (!$this->notinvoke) {' . $this->returnSymbol;
        $bodyMiddle .= $this->doubleTab . $this->tripleTab . '$sBuffer = $this->_communicator->invoke($requestPacket,$this->_iTimeout);' . $this->returnSymbol;
        $bodyMiddle .= $this->quardupleTab . '}' . $this->doubleReturn;

        $bodyMiddle .= $this->tripleTab . '} else {' . $this->returnSymbol;
        $bodyMiddle .= $this->quardupleTab . '$sBuffer = $this->responseBuffer;' . $this->returnSymbol;
        $bodyMiddle .= $this->tripleTab . '}' . $this->returnSymbol;
